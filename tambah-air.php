...
if(isset($_POST['submit'])):	
	...
	if(empty($errors)):
		...
		$handle = $pdo->prepare('INSERT INTO air (no_kalung, ciri_khas, tanggal_input) VALUES (:no_kalung, :ciri_khas, :tanggal_input)');
		$handle->execute( array(
			'no_kalung' => $no_kalung,
			'ciri_khas' => $ciri_khas,
			'tanggal_input' => date('Y-m-d')
		) );
		$sukses = "air no. <strong>{$no_kalung}</strong> berhasil dimasukkan.";
		$id_air = $pdo->lastInsertId();
		
		// Jika ada kriteria yang diinputkan:
		if(!empty($kriteria)):
			foreach($kriteria as $id_kriteria => $nilai):
				$handle = $pdo->prepare('INSERT INTO nilai_air (id_air, id_kriteria, nilai) VALUES (:id_air, :id_kriteria, :nilai)');
				$handle->execute( array(
					'id_air' => $id_air,
		...
		redirect_to('list-air.php?status=sukses-baru');		
	...
	<div class="container clearfix">
		...
		<div class="main-content the-content">
			...
			<?php endif; ?>			
				...
				<form action="tambah-air.php" method="post">
					...
					<div class="field-wrap clearfix">
						<button type="submit" name="submit" value="submit" class="button">Tambah Air Mineral</button>
					</div>
				</form>
